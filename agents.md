...
2.  **Platform-Managed Cloud Storage**: All user data is synced to the application's own S3-compatible object storage (Cloudflare R2), organized by user and vault ID. Users do not need to manage their own storage credentials.
...
-   **Respect the Platform-Managed Storage Principle**: All note content is stored in the application's R2 bucket, organized by `{userId}/{vaultId}/`. Do not build features that assume user-provided storage credentials. The central D1 database stores metadata only (user accounts, vault memberships).
